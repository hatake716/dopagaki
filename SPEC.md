...
- 背景は黒。アプリテーマは既定ダークで、ブランドバーのタップでライト / ダークをアニメーション付きで切り替えられる（設定は保存）。両サイトが「デバイスの設定に従う」表示設定なら prefers-color-scheme 経由で追従する
...
- WebView 内の prefers-color-scheme は uiMode の変化に追従する。サイト側の表示設定が「ライト/ダーク固定」の場合はサイト設定が優先される
