...
```
┌──────────────────────┐ ← パンチホール分だけ黒い余白
│ ▣ dopagaki           │ ← ブランドバー（20dp、アイコン + アプリ名）
├──────────────────────┤
...
- 背景は黒。アプリテーマは既定ダークで、ブランドバーのタップでライト / ダークをアニメーション付きで切り替えられる（設定は保存）。サイトが prefers-color-scheme に追従しない場合も、描画テーマを検出して invert フィルタで**強制的に**切り替える
...
| タップ | ブランドバー | ライト / ダークをアニメーションで切り替え |
...
- ブランドバーのタップで `AppCompatDelegate.setDefaultNightMode` を YES/NO で切り替え、`configChanges` に `uiMode` があるため Activity は再生成されない（WebView の状態を保つ）
...
16. **ブランドバータップでテーマ切り替え**（2026-08-30 追加要望）: ライト / ダークをアニメーション付きで切り替え、設定を保存する。既定はダーク（§2 の「ダーク固定」を置き換え）
17. **テーマの強制反転**（2026-08-30 追加要望）: サイト側の表示設定が固定でも切り替わるよう、描画テーマと希望が食い違うペインには invert フィルタを適用する（§6 参照）。サイト自身のテーマ設定を書き換えるわけではないので、反転画面の配色はサイトネイティブのテーマと完全には一致しない
